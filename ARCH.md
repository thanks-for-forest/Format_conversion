...
> 版本：v0.23.1　关联 PRD v0.3.1　更新日期：2026-09-08
...
| v0.23.2 | 切片 12c：**转换结果增强**（借鉴 TinyPNG 两个点子）。① 体积变化数据行：`ConvertResult` 新增 inSize/outSize props，完成后显示「30 KB → 7 KB · 减少 76%」——本地转换取 blob.size、服务端取 TaskInfo.out_size（已有字段）；增大时中性展示「增大 X%」（doc→pdf 场景），不炫耀不隐藏。② 图片前后对比滑块：新组件 `CompareSlider`——结果图垫底、原图绝对定位覆盖 + `clip-path: inset(0 X% 0 0)` 控制露出、透明 range 全覆盖拖动、白色分割线 + 「原图/转换后」角标；仅图片互转组合显示（IMG_IN×IMG_OUT 白名单判定），文档/音视频无并排可比性不显示。SingleConverter 接线：outSize/beforeUrl state，beforeUrl blob URL 与 downloadUrl 同一套「创建即记录、重置即 revoke」生命周期；E2E：png→jpg 本地转换实测数据行 + 滑块双 img + range 全就位 | 结果数据行是免费工具最有说服力的价值证明（TinyPNG 转化率核心武器），成本仅两个已有字段；对比滑块选 clip-path 双 img 叠加而非 canvas（零 canvas 依赖、跨域 URL 也可显示——服务端结果直接用下载 URL）；beforeUrl 生命周期复用 v0.22.5 的「effect 内创建+cleanup 撤销」思想改为事件内创建+reset 回收（事件驱动场景无 StrictMode 问题） |