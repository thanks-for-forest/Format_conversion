...
> 版本：v0.23.16　关联 PRD v0.3.1　更新日期：2026-09-08
...
| v0.23.8 | 切片 12i：徽章轨道升级为**持续公转**（用户提议：模块围绕图片转动）。双层反向旋转：轨道层 `.orbit-spin`（60s 线性 0→360°）带动全部徽章公转，每枚徽章 `.orbit-counter`（同 keyframes + `reverse`）反向自转抵消——文字始终保持水平；徽章椭圆定位移交 wrapper（translate 定位），span 只承担 counter 动画。验证：DOMMatrix 读 getComputedStyle transform，间隔 5s 角度 17°→49°（=6°/s，与 60s/圈吻合）+ 截图文字水平。替代 12h 的错峰浮动（公转本身即动态，浮动叠加需三层嵌套且 counter 与浮动需合并 keyframes，复杂度不划算） | 双层反向旋转的经典约束：**counter 动画与容器动画必须同 duration/同 easing/同 delay（不可错峰）**，否则文字歪斜；错峰浮动与公转互斥（前者各转各的、后者全局同步）；DOMMatrix 读 computed transform 是动画断言的可靠手段（比截图对比精确）；reduced-motion 下两层动画同时停，徽章静止在椭圆位 |
| v0.23.9 | 轨道从椭圆改**正圆**（用户提议：运动轨迹为圆形）。关键认知：百分比定位的 x/y 分别相对容器宽/高——容器非正方时等比半径在像素上仍是椭圆；故 `aspectRatio` 改 1/1（实测 420×420）+ `r = 45` 常量半径 → 真正的像素正圆轨迹。方形裁切后插画聚焦「PDF 顶部居中 + 女孩居中 + 云朵底部」，构图反而更完整 | 百分比定位的隐藏坑：等比 ≠ 等像素，圆形轨迹的前提是容器正方；cover 裁切方形后主体（居中构图插画）展示效果更佳 |
| v0.23.10 | 轨迹范围扩大（用户反馈：徽章运动不要接触图片）。几何核算：45° 对角方向插画半对角 ≈199px 而原轨道半径 189px——轴向看着有间隙但对角必接触；改 `r = 45% → 49%`（205.8px）+ 插画 `inset 16%/17% → 22%/24%`（218×235，半对角 162px），徽章内缘与插画间隙 ≈10px 全程无接触。实测截图确认 | 「看着不接触」不可靠：方形插画的**对角方向**才是最近点（半对角 > 半宽/半高），轨道半径必须按半对角 + 徽章半宽校核；调轨道参数时插画尺寸要联动缩小，二者是一组约束 |
| v0.23.11 | **隐私声明诚实性审查**（用户质询：footer 声明是否与代码事实一致）。逐条核对：① 「仅在服务器临时处理」**不准确**——实际是混合模式（图片/压缩包/MD 浏览器本地完成不上传；仅文档/音视频/Office 预览走服务端），且与首页亮点卡「浏览器本地完成」自相矛盾；② 「下载后立即删除」→ 精确语义是 BackgroundTask 在**开始下载时**删文件 + beat 每 10 分钟清扫「终态后 60 分钟未下载」的文件（SWEEP_FILE_RETENTION_MIN=60）；③ 「不留存」→ 文件不留存成立，但任务**元数据**（原始文件名/大小/时间）留存 7 天（SWEEP_ROW_RETENTION_DAYS=7）后删行；④ 「不用于其他用途」成立（代码无分析/分享逻辑，日志只记数量）。修正 footer 与隐私亮点卡文案：「图片与压缩包转换在您的浏览器本地完成，文件不上传；服务端处理的文件在开始下载时即删除（未下载的 1 小时后自动清除），转换记录 7 天后删除，不用于其他用途」 | 隐私声明的每个数字必须能指到具体代码常量（60 分钟 = SWEEP_FILE_RETENTION_MIN、7 天 = SWEEP_ROW_RETENTION_DAYS）；「本地优先」与「仅服务器处理」混用是混合架构站点最常见的自相矛盾；诚实披露元数据留存窗口比笼统「不留存」更可信，也是合规（个人信息保护）的正确姿势 |
| v0.23.12 | 切片 12j：产品亮点卡改**玻璃拟态 + 悬停显隐**（用户提议：玻璃卡片悬停，触碰时显示具体内容）。分段背景改淡彩渐变（#dff2ea→#e8ecff→#dcecf9，绿蓝过渡呼应插画与品牌绿——玻璃卡必须有彩色背景才可见磨砂效果）；`.glass-card`：rgba 白 0.55 + backdrop-filter blur(12px) + 白细边，hover 加深至 0.88 + 上浮 4px + 阴影；描述折叠：`.glass-desc` 用 **grid-template-rows 0fr→1fr** 动画（比 max-height 技巧干净，无需猜高度）+ opacity 过渡，hover 展开；**触屏设备（@media hover: none）描述常显**（无 hover 能力不能藏内容）；卡上方加提示「将光标移到卡片上查看详情」。验证：样式表规则断言 5 条全加载（CDP hover 无法模拟 :hover 状态属工具限制，真实鼠标行为由用户确认）；折叠态 computed rows 6px（p paddingTop 残留，opacity 0 下不可见不修） | 玻璃拟态的三要素：半透明白 + backdrop blur + 背景必须有色彩（白底上毛玻璃不可见）；grid-rows 0fr 折叠比 max-height 优雅（无需猜内容高度）但 grid item 的 overflow:hidden 必须设在内容元素上；悬停藏内容必须配套触屏常显（@media hover:none）与操作提示文案，否则移动端/首访用户看不到功能 |
| v0.23.13 | 亮点卡改**田字排版**（用户提议）：`auto-fit minmax(240px,1fr)` 在 760px 容器下呈现 3+1 不对称 → `.glass-grid` 固定 2×2，窄屏（≤640px）降为单列。网格样式从内联移入 globals.css（媒体查询需全局类，延续 btn-primary/glass-card 模式） | 田字是对称美学也是四项能力的自然分组；auto-fit 的「自适应列数」在固定容器宽下反而产生奇数列失衡——布局意图明确时固定列数优于自适应 |
| v0.23.14 | 切片 12k：核心流程收尾打磨（四项）。① **favicon**：`app/icon.svg`（Next 约定自动生成）——品牌绿圆角方块 + 白色双向箭头（⇄ 表达「转换」），浏览器标签页识别；② **结果卡片化**：ConvertResult 成功态改浅绿卡（brand-soft 底 + ✅ + 数据行 + 对比滑块收拢），错误态改浅红卡（#fef2f2/#fecaca），结果从裸文本升级为视觉块；③ **批量进度条**：BatchQueue 运行中显示品牌绿进度条（processedCount/total，width transition .3s）+ 「已处理 N/M · 完成 X」计数；④ **清单类型图标**：PickedFiles 按扩展名映射 emoji（🖼️📄📊📽️🎵🎬🗜️📁），清单项一眼识别文件类别。E2E：png→jpg 实转验证结果卡/数据行/滑块/图标全就位 | favicon 是每个访客都可见的最低成本品牌触点（Next app/icon.svg 约定零配置）；结果区是转换流程的「终点体验」，卡片化后成功/失败状态一眼可辨；进度条把黑盒队列变成可感知的过程；emoji 图标零资源成本覆盖 30+ 扩展名识别 |
| v0.23.15 | **全矩阵冒烟测试**（用户质询「所有类型的格式转换都测试过了吗」——此前仅有分类抽测，无逐组合实证）。① 补样本：新增 `testdata/generate_samples.py`（可重复运行）生成 20 个缺失样本——PIL 出 jpg、openpyxl 出 xlsx、**手工构造 odp zip**、soffice 派生 odt/xls/ods/pptx/ppt/doc、ffmpeg 截 5 秒出 wav/flac/aac/ogg/m4a 与 mov/mkv/webm/avi；② 服务端 67 组合：新增 `testdata/smoke_server.py` 回归脚本（POST /api/convert → 轮询终态 → 下载并校验字节数=out_size，4 并发，结果落 smoke_server_results.json）——**67/67 通过**；③ 本地 13 组合：浏览器 UI 逐一实转（12 图片 Canvas 互转 + md→html），断言结果卡「转换方式：本地（文件未上传）」+ 体积行——**13/13 通过**；④ 口径修正：图片组合实为 **12**（5 源×3 目标仅 png/jpg/webp 3 组同格式需排除），全站组合总数 **80** 而非此前文档口径的 83 | 样本生成沉淀为脚本=未来回归零成本；手工构造 ODF 最可靠（LibreOffice 读自家格式容错最高，pptx 最小化 XML 反而易踩严格校验）；`--convert-to doc` 需显式 filter `doc:MS Word 97` 否则静默失败；文档/首页「80+」文案与实测 80 一致（承诺不虚标）；browser_evaluate 只回传**最后表达式的值**（IIFE+return 得 undefined）；React key 重挂载（pickSeq 变化）后旧元素 ref 必失效——自动化操作 refs 必须取自最近一次快照 |
| v0.23.16 | **全站文案诚实性审查第二轮**（用户质询「免费 · 无需注册」及前端文字正确性）。逐条对照代码事实核查全部用户可见文案，**4 处失实已修**：① 首页副文案「服务端转换的文件处理完即删」——与代码不符（删除点=开始下载时 BackgroundTask；未下载 1 小时被 beat 清扫），且与已修正的 footer/隐私卡自相矛盾 → 改「在开始下载时即删（未下载的 1 小时后自动清除）」；② 落地页 4 类模板（doc/pdf、audio、video、mp3 提取）12 处「处理完即删」同款问题 → 统一「下载即删 / 开始下载时即从服务端删除」；③ 音频落地页「保留原始音质」——有损→有损转码（mp3→aac）必有损，属夸大 → 删除该短语；④ 首页「28 种格式」——源格式实际 29（5 图+12 文档+6 音+5 视+md，FileDrop accept 含 md），与「80+ 组合含 md 组合」口径不自洽 → 改「29 种」；顺手补图片互转卡漏列的 GIF。**核查通过项**：「免费」「无需注册」「匿名即可使用，登录后每日额度更高」（匿名/登录双档配额）、「80+ 种转换组合」（实测 80 + 压缩包方向）、「UTC 零点重置」、「25MB 本地阈值」（LOCAL_MAX_BYTES=25MB）、「TAR.GZ 统一转 ZIP 交付」、zip 两页文案、footer 精确表述 | 文案审查要覆盖**全部页面**而非只审被质询那句——本次 12 处落地页残留全靠全站 grep 兜底（单修首页会漏）；「处理完即删」这类模糊话术与代码精确语义（删除时点）的差距是隐私文案重灾区，SEO meta description 同样受审查（它是搜索结果里的承诺）；「保留原始音质」是转码类产品的典型夸大，无损/有损方向性结论（上轮答疑）直接证伪；数字口径必须全局自洽（格式数含 md 而组合数也含 md 组合） |