...
> 版本：v0.23.3　关联 PRD v0.3.1　更新日期：2026-09-08
...
| v0.23.4 | 切片 12e：**hero 自然场景背景图**。文生图（text_to_image，landscape_16_9，prompt：竹林+草地+晨光金色光束的自然场景摄影风）→ `public/hero-bg.jpg`；`SiteFrame` hero 接入 next/image（fill + objectFit cover + **priority** 首屏 LCP 优先）+ 白色渐变遮罩（顶部 .82 → 底部 .92，向页面底色过渡）保证墨色标题/徽章可读。**已知状态**：生成 API 当前对自动化请求持续返回「generating」占位图（等待 2.5 分钟未出成品），仓库内暂为占位；用户可在浏览器打开生成 URL 刷新等出图后保存为 `frontend/public/hero-bg.jpg` 跳过（覆盖即生效，代码无需改动） | 背景图 vs 纯渐变：TinyPNG 用自然场景照片营造「友好自然」情绪，白卡+遮罩压在其上层次分明；next/image fill+priority 是 Next 首屏大图的标准做法（自动优化尺寸）；遮罩下浅上深的方向选择依据标题在顶部、白卡压在底部边缘的布局事实 |