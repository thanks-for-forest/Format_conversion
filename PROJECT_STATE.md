...
- 阶段：**实现阶段（切片 6b 完成：sitemap/robots，SEO 引流闭环）**
- 状态：切片 6b 已跑通——`app/sitemap.ts` 构建期生成 14 条 URL（首页+12 落地页+登录）、`app/robots.txt` 指向 sitemap；站点地址 `SITE_URL` 构建期 env 可配（生产换正式域名）；另建本地测试素材库 `testdata/`（12 个真实样例文件覆盖图片/音视频/文档/压缩包，魔数验证，gitignore 不入库）；eslint/build 全绿，dev 实测两个响应内容正确
