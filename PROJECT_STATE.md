...
- 阶段：**实现阶段（切片 6a 完成：SEO 落地页）**
- 状态：切片 6a 已跑通——`/convert/[a-to-b]` 12 个静态组合页（generateStaticParams + dynamicParams=false，非法 slug 404）、generateMetadata 按组合生成标题描述、页面含介绍/步骤/11 个内链互链；组件重构：Converter（锁定格式支持）+ SiteFrame（useQuota 单实例骨架）抽出，首页瘦身至 20 行；eslint/build 全绿；浏览器验证：png-to-jpg 本地转换计次闭环、异格式拒绝、404、首页回归
...
1. 切片 6b：account 页（我的配额）或 sitemap.xml/robots.txt（SEO 收尾）
