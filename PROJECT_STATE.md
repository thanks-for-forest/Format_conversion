...
- 阶段：**实现阶段（切片 6c 完成：账户配额页）**
- 状态：切片 6c 已跑通——`/account` 页展示次数/流量进度条、单文件上限、重置时刻（UTC 转本地）；匿名态展示用量 + 登录引导与两档额度对比；AuthBar 增加「配额」入口、邮箱链接到账户页；纯前端切片无后端改动；eslint/build 全绿；浏览器验证匿名态（1/5 次、0/100MB、重置 08:00 本地时间）
...
1. docker-compose 部署切片：含 sweeper 定时清扫（超时未下载的临时文件与过期记录）
2. 切片 7：压缩包（fflate 本地）/ 文档（LibreOffice）/ 音视频（ffmpeg.wasm + 服务端）新类别
3. 运营向：登录态浏览器全链路人工验证（真实邮箱）、OG 标签/分享卡片
