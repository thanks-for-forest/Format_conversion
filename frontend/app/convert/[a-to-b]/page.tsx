...
function buildCopy(combo: Combo): {
  ...
  if (combo.category === "doc" && combo.to === "pdf") {
    return {
      ...
      description: `免费在线把 ${F} 文档转换成 PDF：服务端 LibreOffice 转换，文件下载即删、不留存，无需注册即可使用。`,
      intro: `免费在线把 ${F} 文档转换成 PDF：上传后在服务端用 LibreOffice 转换，文件下载即删，无需注册即可使用。`,
      steps: [
        `选择或拖入 ${F} 文档`,
        "点击「开始转换」，服务端转换完成后自动下载 PDF 文件",
        "文件在开始下载时即从服务端删除，不会留存",
      ],
    };
  }
  ...
  if (combo.category === "audio") {
    return {
      ...
      description: `免费在线把 ${F} 音频转换成 ${T}：服务端 ffmpeg 转换，文件下载即删、不留存，无需注册即可使用。`,
      intro: `免费在线把 ${F} 音频转换成 ${T}：上传后在服务端用 ffmpeg 转换，文件下载即删，无需注册即可使用。`,
      steps: [
        `选择或拖入 ${F} 音频文件`,
  ...
  if (combo.category === "video") {
    if (combo.to === "mp3") {
      return {
        ...
        description: `免费在线把 ${F} 视频中的音频提取成 MP3：服务端 ffmpeg 转换，文件下载即删、不留存，无需注册即可使用。`,
        intro: `免费在线把 ${F} 视频中的音频提取成 MP3：上传后在服务端用 ffmpeg 提取音轨，文件下载即删，无需注册即可使用。`,
        steps: [
          `选择或拖入 ${F} 视频`,
    ...
    return {
      ...
      description: `免费在线把 ${F} 视频转换成 ${T}：服务端 ffmpeg 转码，文件下载即删、不留存，无需注册即可使用。`,
      intro: `免费在线把 ${F} 视频转换成 ${T}：上传后在服务端用 ffmpeg 转码，文件下载即删，无需注册即可使用。`,
      steps: [
        `选择或拖入 ${F} 视频`,
        `点击「开始转换」，服务端转码完成后自动下载 ${T} 文件`,
        "视频转码耗时较长，请耐心等待；文件在开始下载时即从服务端删除",
      ],
    };
  }
  return {
